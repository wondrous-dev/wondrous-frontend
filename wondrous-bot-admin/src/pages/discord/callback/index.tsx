...
function DiscordCallback() {
  ...
  const [discordSignupLogin, { data, loading }] = useMutation(
    USER_DISOCRD_SIGNUP_LOGIN
  );
  ...
  const returnToPage = useCallback(() => {
    navigate(
      `/login?discordConnectError=true${
        parsedState?.collabInvite
          ? `&collabInvite=${parsedState?.collabInvite}`
          : ''
      }`
    );
  }, [state, navigate]);
  ...
  useEffect(() => {
    if (code && state && !data?.discordSignupLogin && !loading) {
      ...
      if (
        ...
        parsedState.callbackType ===
          DISCORD_CONNECT_TYPES.connectOnboardingDao ||
        ...
        connectUserDiscord({
          ...
          .then((result) => {
            if (
              ...
              parsedState.callbackType ===
              DISCORD_CONNECT_TYPES.connectOnboarding
            ) {
              navigate(
          ...
          .catch((err) => {
            console.log(
              'Error updating discord',
              err?.graphQLErrors[0]?.extensions.errorCode
            );
            const alreadyExists =
              err?.graphQLErrors &&
              err?.graphQLErrors[0]?.extensions.errorCode ===
                GRAPHQL_ERRORS.DISCORD_USER_ALREADY_EXISTS;

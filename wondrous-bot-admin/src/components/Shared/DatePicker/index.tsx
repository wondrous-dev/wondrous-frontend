...
const DateRangePicker = ({
  ...
  const startDateString = dateRange?.startAt?.format(MONTH_DAY_FULL_YEAR) || "";
  ...
  return (
    <ClickAwayListener onClickAway={handleOnClose} mouseEvent="onMouseDown">
      <div>
        ...
        >
          <Box width="302px" borderRadius="16px" border="1px solid #000000" overflow="hidden">
            <Box gap="10px" padding="12px 25px" display="flex" bgcolor="#2A8D5C">
              <TextField
                type="text"
                name="start date"
                ...
                InputProps={textFieldInputProps({
                  ...
                  onClick: () => setDateRange({ ...dateRange, startAt: null }),
                })}
            ...
            >
              <DayPickerRangeController
                renderWeekHeaderElement={(day) => (
                  <Typography fontWeight="700" fontSize="13px" color={"#2A8D5C"}>
                    {day?.[0]}
                  </Typography>
                )}
                ...
                onDatesChange={({ startDate, endDate }) => setDateRange({ startAt: startDate, endAt: endDate })}
                ...
                renderCalendarInfo={() => (
                  <Grid container padding="0 25px">
                    <Grid container item justifyContent="space-between" padding="12px 0" borderTop={`1px solid black`}>
                      <ButtonBase
                        onClick={() => {

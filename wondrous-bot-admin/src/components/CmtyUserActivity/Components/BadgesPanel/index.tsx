...
const Card = ({ title, id, assetSrc, link }: ICardProps) => {
  return (
    <Grid
      item
      display="flex"
    ...
    >
      <Box width="100%" height="100%" position="relative" flex="1">
        {link && <MintingStatus />}
        <CardImage src={assetSrc} alt={`An image displaying user minted NFT`} />
      </Box>
      <Box
        display="flex"
        gap="14px"
        ...
        flexDirection="column"
      >
        <PanelTitle $fontSize="15x">{title}</PanelTitle>
        <StyledButton $reverse={!!link}>{link ? "View Badge" : "Mint now"}</StyledButton>
      </Box>
    </Grid>
  );
};
  ...
  return (
    <Grid display="flex" flexDirection="column" gap="18px" width="100%">
      <Box display="flex" alignItems="center" gap="8px" justifyContent="flex-start">
        <PanelCount>
          <PanelTitle>8</PanelTitle>
        </PanelCount>
        <PanelTitle>badges</PanelTitle>
      </Box>
      <CardsList config={CONFIG} />

...
const WonderBackendA = new MockWonderBackend({ "fooProject" : ProjectC, "barProject": ProjectE })
...
describe('SyncProjectWithWonder functions properly', () => {
  ...
  it('throws on unable to sync', () => {
    expect(() => SyncProjectWithWonder(ProjectA, WonderBackendA)).to.throw("Unable to update out of sync project fooProject in backend.");
  });
});

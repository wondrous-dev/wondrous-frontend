import { findNode, getEditorString, getPluginType, PlateEditor, Value } from '@udecode/plate-core';

import { ElementTypes } from 'components/RichTextPlate/typescript/plateTypes';
import { floatingLinkActions } from '../components/FloatingLink/floatingLinkStore';
import { TLinkElement } from '../types';

export const triggerFloatingLinkEdit = <V extends Value>(editor: PlateEditor<V>) => {
  const entry = findNode<TLinkElement>(editor, {
    match: { type: getPluginType(editor, ElementTypes.ELEMENT_LINK) },
  });
  if (!entry) return;

  const [link, path] = entry;

  let text = getEditorString(editor, path);

  floatingLinkActions.url(link.url);

  floatingLinkActions.newTab(link.target === undefined);

  if (text === link.url) {
    text = '';
  }

  floatingLinkActions.text(text);

  floatingLinkActions.isEditing(true);

  return true;
};

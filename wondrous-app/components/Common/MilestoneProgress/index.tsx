...
export const MilestoneProgress = (props) => {
    ...
    useEffect(() => {
        ...
        if (data?.getPerStatusTaskCountForMilestone) {
            ...
            setTaskTotal(Object.values(getPerStatusTaskCountForMilestoneData)?.filter(i => typeof (i) === "number")?.reduce((a: number, b: number) => a + b, 0) ?? 0)
            setTaskCompleted(getPerStatusTaskCountForMilestoneData?.completed + getPerStatusTaskCountForMilestoneData?.archived)
        }

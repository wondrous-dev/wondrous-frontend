...
export const TaskLikeIcon = (props) => {
  ...
  const [colorFill, setColorFill] = useState(greyColors.grey57);

  const handlePointerEnter = (event) => {
    setColorFill(redColors.red400);
  };

  const handlePointerLeave = (event) => {
    setColorFill(greyColors.grey57);
  };
  ...
  return (
    ...
    >
      <path
        ...
        stroke={liked ? redColors.red400 : colorFill}
        strokeLinecap="round"
        strokeLinejoin="round"
        fill={liked ? redColors.red400 : 'none'}
      />
    </svg>
  );
};
